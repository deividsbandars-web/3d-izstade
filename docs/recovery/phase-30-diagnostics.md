...
- the authoritative implementation already existed in `agentExecutor.ts`
- the active import path was the normal `.js` specifier from `src/backend/agents/agentsApplicationService.ts`
- there were no special package-script, worker, dynamic-import, or event-subscriber paths depending on a physical `agentExecutor.js` source file
...
- controller/expo/distribution/platform/agents files scanned: `61`
- domain duplicate warnings: `8`
...
- agents duplicate warnings fell from `9` to `8`
- distribution duplicate warnings remain `0`
- platform duplicate warnings remain `0`
- `agentExecutor.js` no longer appears in duplicate warnings

## Files changed

- `src/backend/agents/execution/agentExecutor.js` (removed)
- `docs/recovery/phase-30-agents-execution-duplicate-burn-down-1.md`
- `docs/recovery/phase-30-diagnostics.md`
...
- no `agentExecutionLoop.js` removal in this phase
- no runner cleanup
- no planner cleanup
