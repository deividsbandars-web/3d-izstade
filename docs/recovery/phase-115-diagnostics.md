...
Intent:
- re-rank the remaining backend duplicate candidates after the websiteScraper stabilization pass
...
- `src/backend/marketplace/installService.ts/.js`
...
Not selected now:
- business generator pair: DB + LLM + workflow orchestration
- growth lead capture pair: reopens stabilized leads/growth seam
- outreach email pair: broader delivery/provider behavior
- billing usage cluster: recently stabilized domain, no new signal
- events infra cluster: broad shared infra fanout
- AI/governance cluster: cross-cutting control surface
- logger pair: very broad import fanout
...
- selected target: `MARKETPLACE INSTALL SERVICE DUPLICATE REVIEW`
