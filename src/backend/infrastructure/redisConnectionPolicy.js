...
function isLocalOrPrivateRedisHost(hostname) {
...
function isUpstashLikeHost(hostname) {
  return hostname.toLowerCase().includes('upstash');
}

function hasForbiddenTlsQueryOption(url) {
...
export function resolveRedisConnectionPolicy(rawRedisUrl, rawEnv = process.env) {
...
export function getRedisTlsOptions(policy) {
  return policy.tls
