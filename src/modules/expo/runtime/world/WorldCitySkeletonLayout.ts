...
export function buildMediaWallSurfaces(districtCount: number, districtStride: number): CityScreenSurface[] {
  return Array.from({ length: Math.max(3, districtCount) }, (_, districtIndex) => {
    ...
    const heroSurfaces = [
      {
        id: `media-surface-left-hero-${districtIndex}`,
        position: [-468, 136, baseZ - 69],
        size: [124, 168, 3],
        color: '#0f172a',
        glowColor: '#67e8f9',
      ...
      {
        id: `media-surface-right-hero-${districtIndex}`,
        position: [468, 132, baseZ - 101],
        size: [124, 162, 3],
        color: '#111827',
        glowColor: '#c084fc',
        role: 'hero-wall' as const,
    ...
    return [
      ...heroSurfaces,
      {
        id: `media-surface-left-support-${districtIndex}`,
        position: [-468, 84, baseZ + 39],
        size: [110, 104, 3],
        color: '#0f172a',
        glowColor: '#93c5fd',
      ...
      {
        id: `media-surface-right-support-${districtIndex}`,
        position: [468, 84, baseZ + 23],
        size: [110, 104, 3],
        color: '#0f172a',
        glowColor: '#93c5fd',
        role: 'support-wall' as const,
...
export function buildTowerScreenSurfaces(towers: CityTower[]): CityScreenSurface[] {
  return towers.flatMap((tower) => {
    ...
    const sideSurface: CityScreenSurface = {
      id: `${tower.id}-side-surface`,
      position: [
        tower.position[0],
        tower.baseSize[1] * (isHeroTower ? 0.56 : 0.52),
        tower.position[2] + ((tower.position[0] < 0) ? faceOffset : -faceOffset),
      ],
      size: [
        isHeroTower ? 44 : 32,
        isHeroTower ? 124 : 92,
      ...
      color: '#0f172a',
      glowColor: isHeroTower ? '#fbbf24' : tower.crownColor,
      role: 'tower-side',
      type: 'tower-side',
    };

    const crownSurface: CityScreenSurface = {
      id: `${tower.id}-crown-surface`,
      position: [
        tower.position[0],
        tower.baseSize[1] + tower.upperSize[1] - (isHeroTower ? 30 : 24),
        tower.position[2] + ((tower.position[0] < 0) ? faceOffset + 2 : -(faceOffset + 2)),
      ],
      size: [
        isHeroTower ? 62 : 44,
        isHeroTower ? 48 : 36,
    ...
    return [sideSurface, crownSurface];
  });
}

export function buildScreenSockets(surfaces: CityScreenSurface[]): CityScreenSocket[] {
  return surfaces.map((surface) => {
    const depthOffset = surface.size[2] * 0.62;
    const isLeftSide = surface.position[0] < 0;

    if (surface.role === 'hero-wall') {
      return {
        color: surface.glowColor,
        frameSize: [surface.size[0] * 0.92, surface.size[1] * 0.9],
        id: `${surface.id}-socket`,
        kind: 'hero_wall',
        position: [surface.position[0], surface.position[1], surface.position[2] + depthOffset],
        rotation: [0, 0, 0],
    ...
    if (surface.role === 'tower-crown') {
      return {
        color: surface.glowColor,
        frameSize: [surface.size[0] * 0.88, surface.size[1] * 0.82],
        id: `${surface.id}-socket`,
        kind: 'tower_crown',
        position: [surface.position[0], surface.position[1], surface.position[2] + (isLeftSide ? depthOffset : -depthOffset)],
        rotation: [0, isLeftSide ? 0 : Math.PI, 0],
        surfaceId: surface.id,
      } satisfies CityScreenSocket;
    }
    ...
    return {
      color: surface.glowColor,
      frameSize: [surface.size[0] * 0.86, surface.size[1] * 0.86],
      id: `${surface.id}-socket`,
      kind: 'tower_side',
      position: [surface.position[0], surface.position[1], surface.position[2] + (isLeftSide ? depthOffset : -depthOffset)],
      rotation: [0, isLeftSide ? 0 : Math.PI, 0],
...
export function buildScreenAssignments(
  ...
  const distributedHeroWalls = pickDistributedSockets(heroSockets, Math.min(4, heroSockets.length));
  const distributedPremiumWalls = pickDistributedSockets(wallSockets, Math.min(6, wallSockets.length));

  assignFromQueue(heroPlacements.length > 0 ? heroPlacements : elitePlacements, distributedHeroWalls, 'hero');
  assignFromQueue(premiumPlacements.length > 0 ? premiumPlacements : elitePlacements, distributedPremiumWalls, 'premium');
