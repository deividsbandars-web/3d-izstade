        ...
        .map((mass) => {
          ...
          return (
            <group key={mass.id} name={`city-mass:${mass.id}`} position={[mass.position[0], 0, mass.position[2]]}>
              ...
              {hasHorizontalCap && (
                <mesh position={[0, mass.size[1] + 0.4, 0]} castShadow>
              ...
              {hasSideInset && (
                <>
                  <mesh position={[-mass.size[0] * 0.24, mass.size[1] * 0.54, 0]} castShadow>
                    <boxGeometry args={[Math.max(8, mass.size[0] * 0.1), Math.max(16, mass.size[1] * 0.24), Math.max(8, mass.size[2] * 0.16)]} />
                    <meshStandardMaterial color="#eef4f8" roughness={0.34} metalness={0.18} />
                  </mesh>
                  <mesh position={[mass.size[0] * 0.24, mass.size[1] * 0.5, 0]} castShadow>
                    <boxGeometry args={[Math.max(8, mass.size[0] * 0.08), Math.max(14, mass.size[1] * 0.2), Math.max(8, mass.size[2] * 0.14)]} />
                    <meshStandardMaterial color="#d8e3ea" emissive="#cfe6f4" emissiveIntensity={0.02} roughness={0.36} metalness={0.16} />
                  </mesh>
                </>
              )}
              ...
              {hasRearSpine && (
                <mesh position={[0, mass.size[1] * 0.68, -mass.size[2] * 0.22]} castShadow>
                  <boxGeometry args={[Math.max(8, mass.size[0] * 0.12), Math.max(18, mass.size[1] * 0.18), Math.max(6, mass.size[2] * 0.1)]} />
                  <meshStandardMaterial color="#e7eef3" emissive="#cfe6f4" emissiveIntensity={0.018} roughness={0.34} metalness={0.14} />
                </mesh>
              )}
              {hasFrontWing && (
                <mesh position={[0, mass.size[1] * 0.34, mass.size[2] * 0.22]} castShadow>
                  <boxGeometry args={[Math.max(12, mass.size[0] * 0.26), Math.max(10, mass.size[1] * 0.12), Math.max(6, mass.size[2] * 0.1)]} />
                  <meshStandardMaterial color="#eef4f8" emissive="#d9eef8" emissiveIntensity={0.018} roughness={0.32} metalness={0.16} />
                </mesh>
              )}
            </group>
          );
