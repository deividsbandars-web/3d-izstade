...
export function buildCityScreenSurfacePool(districtCount: number, districtStride: number): CityScreenSurface[] {
  ...
  const marqueeRightOutwardX = (districtIndex: number) => (districtIndex === 2 ? 590 : 135.5);
  ...
  const marqueeLeftDepthOffset = (districtIndex: number) => (
    ...
    return [
      {
        ...
        position: [-(708 + marqueeLeftOutwardX), 148, baseZ - 82 + marqueeClearanceZ + marqueeLeftDepthOffset(districtIndex)],
      ...
      },
      {
        ...
        position: [
          -968 - sideArrayClearanceX + (districtIndex === 2 ? 70 : 0),
          sideArrayY(districtIndex),
          baseZ + 104 + sideArrayLeftDepthOffset(districtIndex),
        ],
