...
function buildAiReactorCoreMasses(): CityMass[] {
  ...
  const containmentBands: CanonicalPrimitive[] = [470, 820, 1160].map((y, index): CanonicalPrimitive => ({
    color: index === 1 ? '#fff2b8' : '#8ffff4',
    emissive: index === 1 ? '#ffdf8a' : accent,
    emissiveIntensity: index === 1 ? 0.074 : 0.066,
    kind: 'torus',
    metalness: 0.26,
    opacity: index === 1 ? 0.82 : 0.74,
    physics: 'decorative',
    position: [0, y, 0],
    radialSegments: 16,
    radius: index === 1 ? 372 : 330,
