...
export function DoubleSidedScreenSurface({
  backOffset = 0.012,
  depthWrite = false,
  emissiveColor,
  emissiveIntensity = 0,
  ...
  const renderFallbackMaterial = () => (
    <meshStandardMaterial
      color={fallbackColor}
      depthWrite={depthWrite}
      emissive={emissiveColor ?? fallbackColor}
  ...
  return (
    <group position={position}>
      <mesh position={[0, 0, frontOffset]} renderOrder={renderOrder}>
      ...
      <mesh position={[0, 0, -backOffset]} rotation={[0, Math.PI, 0]} renderOrder={renderOrder}>
        <planeGeometry args={size} />
        {url ? (
          <SponsorTextureSurface
            depthWrite={depthWrite}
