...
assert.deepEqual(tvTowerFloor.position, [360, -2, -1030]);
...
assert.equal(findCurrentWorldPhysicsSolidTopY({ x: 360, y: 7, z: -1030 }, 7, physicsAtStart.solids), 7);
assert.equal(findBlockingWorldPhysicsSolid({ x: 360, y: 7, z: -1030 }, physicsAtStart.solids, { radius: 1 }), null);
assert.equal(
  findBlockingWorldPhysicsSolid({ x: 407.5, y: 7, z: -1030 }, physicsAtStart.solids, { radius: 1 })?.solid.id,
  'tower-cluster-television-tower-animated-city-lift:dynamic-cabin-side-wall-right',
);

const currentTvElevator = findCurrentRideableElevator({ x: 360, y: 7, z: -1030 }, 7, 0, runtimeRoutes);
