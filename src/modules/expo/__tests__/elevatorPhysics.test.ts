...
assert.deepEqual(tvTowerFloor.position, [360, -2, -900]);
...
assert.equal(findCurrentWorldPhysicsSolidTopY({ x: 360, y: 7, z: -900 }, 7, physicsAtStart.solids), 7);
assert.equal(findBlockingWorldPhysicsSolid({ x: 360, y: 7, z: -900 }, physicsAtStart.solids, { radius: 1 }), null);
assert.equal(
  findBlockingWorldPhysicsSolid({ x: 407.5, y: 7, z: -900 }, physicsAtStart.solids, { radius: 1 })?.solid.id,
  'tower-cluster-television-tower-animated-city-lift:dynamic-cabin-side-wall-right',
);

const currentTvElevator = findCurrentRideableElevator({ x: 360, y: 7, z: -900 }, 7, 0, runtimeRoutes);
...
assert.equal(findCurrentRideableElevator({ x: 460, y: 7, z: -900 }, 7, 0, runtimeRoutes), null);

const midRideElapsedSeconds = 17;
const detachedByFastCabinMotion = findCurrentRideableElevator({ x: 360, y: 7, z: -900 }, 7, midRideElapsedSeconds, runtimeRoutes);
