...
export function serializeModularHomeQuoteAdminCsv(rows: readonly SupabaseQuoteRow[]): string {
  const header = [
    ...
    'facade_board_profile',
    'facade_board_spacing',
    'facade_board_width',
    'roof',
    'roof_edge_color',
    'roof_gutter_style',
  ...
  const body = rows.map((row) => [
    ...
    getNestedText(row.config, 'facadeBoardProfile'),
    getNestedText(row.config, 'facadeBoardSpacing'),
    getNestedText(row.config, 'facadeBoardWidth'),
    getNestedText(row.config, 'roof'),
    getNestedText(row.config, 'roofEdgeColor'),
    getNestedText(row.config, 'roofGutterStyle'),
    ...
    getNestedText(row.config, 'interiorFloorStyle'),
    getNestedText(row.config, 'wallPanelStyle'),
