...
def log_error(message: str) -> None:
    ...
    unreal.log_error(f"[LT52A-ArchPlace] {message}")
...
def set_prop_safe(target, prop, value):
    try:
        ...
        unreal.log_warning(f"[LT52A-ArchPlace] Could not set {prop}: {exc}")
        return False
...
def clear_previous_generated_actors():
    ...
    for actor in unreal.EditorLevelLibrary.get_all_level_actors():
        ...
        if PROJECT_TAG in tags or label.startswith("lt52a_"):
            ...
            removed += 1
    unreal.log(f"[LT52A-ArchPlace] Removed {removed} prior LT52A actors.")
...
def main():
    try:
        ...
        mesh_paths = sorted(list_assets(MESH_DEST))
        mesh_paths = [path for path in mesh_paths if unreal.EditorAssetLibrary.load_asset(path)]
        if not mesh_paths:
            raise RuntimeError(f"No mesh assets found in {MESH_DEST}")
        ...
        cameras = [
            ("LT52A_Camera_Exterior", unreal.Vector(origin.x - 1450.0, origin.y - 980.0, 280.0), exterior_target, 46.0),
            ("LT52A_Camera_Overview", unreal.Vector(origin.x - 980.0, origin.y - 780.0, 680.0), overview_target, 44.0),
            ("LT52A_Camera_Terrace", unreal.Vector(origin.x + 420.0, origin.y - 1280.0, 220.0), terrace_target, 52.0),
            ("LT52A_Camera_Roof", unreal.Vector(origin.x - 180.0, origin.y - 220.0, extent.z + 980.0), roof_target, 38.0),
        ]
        ...
        status = [
            "# LT52A Architectural Rebuild Placement Status",
